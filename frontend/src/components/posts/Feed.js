...
export default function Feed({ theme }) {
  ...
  return (
    <div
      className={`relative flex flex-col-3 pb-8 mt-2 ${
      ...
      {posts.length === 0 ? (
        ...
        <div className="grid grid-cols gap-1 mt-8">
          {posts.map((postData, index) => {
            const post = postData.post; // Extract the actual post object from postData
            return (
              <div
                key={post._id}
                ...
                {post.media && post.media.length > 0 && (
                  <div className="grid grid-cols-1 gap-2">
                    {post.media.map((media, idx) => {
                      ...
                      // Space for other media types (e.g., video, audio) as the need arises
                      return null;
                    })}
                  </div>
                )}
